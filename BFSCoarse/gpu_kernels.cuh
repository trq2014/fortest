...
template <class T>
__global__ void
setOverloadNodePointerSwap(uint vertexNum, uint *activeNodes, uint *activeNodePointers, T *activeLabel,
...
template <typename T>
__global__ void prKernel_Opt(uint nodeNum, float *valueD, float *sumD, T *isActiveNodeList);
...
bfs_kernelStatic(uint nodeNum, uint *activeNodesD, uint *nodePointersD, uint *degreeD, uint *edgeListD, uint *valueD,
    ...
    streamVertices(nodeNum, [&](uint index) {
        ...
        uint sourceValue = valueD[id];
        uint finalValue;
        //printf("vertex %d degree is %d \n", activeNodesD[index], degreeD[id]);
        for (uint i = 0; i < degreeD[id]; i++) {
...
setOverloadNodePointerSwap(uint vertexNum, uint *activeNodes, uint *activeNodePointers, T *activeLabel,
    ...
    streamVertices(vertexNum, [&](uint vertexId) {
        if (activeLabel[vertexId]) {
            ...
            printf("vertex %d activeLabel %d activeLabelPrefix[vertexId] %d degreeD[vertexId] %d\n"
                    , vertexId, activeLabel[vertexId], activeLabelPrefix[vertexId], degreeD[vertexId]);
...
template <typename T>
__global__ void
prKernel_Opt(uint nodeNum, float *valueD, float *sumD, T *isActiveNodeList) {
    streamVertices(nodeNum, [&](uint index) {
